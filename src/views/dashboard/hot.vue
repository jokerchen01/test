<template>
  <section class="hotPage" @click="isShowTip = false">
    <div class="tagcloud-all" ref="tagcloudall">
      <a
        v-for="(item, i) in tagList"
        :key="i"
        @click.stop="infClick(item)"
    ...
    <div class="tipPop" v-if="isShowTip" @click.stop="">
      <div class="top_box">
        <p class="title">{{ nameTitle }}</p>
        <img
          src="@/assets/images/close_pup.png"
          class="img"
          @click.stop="isShowTip = false"
        />
      </div>
      <div class="container">
        <p v-for="(v, i) in nameList" :key="i" class="text">
          <span class="circle"></span>
          <span class="lable">{{ v }}。</span>
        </p>
      </div>
    </div>
  </section>
...
export default {
  ...
  methods: {
    getHotword() {
       ...
       this.$api.home.reqHotWord().then((res) => {
        this.$nextTick(() => {
          this.query(res.data);
        });
      });
    ...
    infClick(v, i) {
      ...
      this.currentIndex = i;
      this.nameTitle = v.name;
      this.nameList = v.content;
    },
    // 生成随机数
    getRandomNum() {
      let arr = ["#FFC787", "#FFFFFF", "#FFC787", "#FFFFFF"];
...
.tipPop {
 ...
 right: 160px;
  ...
  background: rgba(2, 43, 69, 0.88) !important;
  color: #c6e6fa !important;
