              ...
              <div v-for="(v, i) in processList" class="process_item" :key="i">
                ...
                <div>
                  ...
                  <div class="name">
                    ...
                    <span class="text" v-if="v.message">{{
                      v.message || "无"
                    }}</span>
                    <span class="text" v-else>{{ v.operate || "无" }}</span>
                  </div>
                </div>
              </div>
            </section>
...
export default {
  ...
  methods: {
    close() {
      this.$emit("showTable", this.$parent.types);
    ...
    getProce(val) {
      ...
      if (val.eventSource == "区城运平台") {
        if (this.detail.eventStatus == "已归档") {
          this.$api.statePup
            .getProcessList({
              uuid: this.detail.id,
            ...
            .then((res) => {
              ...
              res.data.forEach((v) => {
                if (v.annexUrl) {
                  let arr = v.annexUrl.split(",");
                  this.annexUrl = arr.map((v) => {
                    return v.replace(
                      "https://oss-chengdu.jinnq.com:9001",
                      "/api-sthjImg"
                    );
                  });
                }
