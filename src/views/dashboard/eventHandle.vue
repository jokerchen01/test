  ...
  <section @click.stop class="event-handle-container">
    ...
    <div class="event-box">
      <section
        ...
        v-loading="isLoading"
      >
        <div ref="scroll" :data="list" :class-option="eventOption(1)">
          <div
            class="event-item"
            :class="[v.eventLevel == '一般' ? 'normal' : 'no-normal']"
            v-for="(v, i) in list"
            :key="i"
            @click="eventClick(v)"
          >
            <div
              v-if="v.eventStatus == '超期' || v.eventStatus == '逾期'"
              class="event-status-label"
            >
              <span class="label">{{ v.eventStatus }}</span>
            </div>
            <div class="event-title">
              <div class="lable">{{ v.eventName }}</div>
              <div
            ...
            <div class="event-content">
              <div class="con-nav">
                ...
                <div class="con-value">{{ v.eventDesc }}</div>
              </div>
              <div class="con-nav">
                <img
                  class="con-label"
                  :src="require('@/assets/images/event_hand_adress.png')"
                  alt=""
                />
                <div class="con-value color-1">{{ v.place }}</div>
              </div>
              <div class="con-nav">
                <img
                  class="con-label"
                  :src="require('@/assets/images/event_hand_time.png')"
                  alt=""
                />
                <div class="con-value color-1">{{ v.eventTime }}</div>
              </div>
            </div>
            <div class="event-bottom">
              <div class="source">{{ v.flowSource }}</div>
            </div>
          </div>
        </div>
      </section>
    ...
    <eventDetail ref="detail"></eventDetail>
  </section>
</template>
  ...
  .event-list {
    ...
    .event-item {
      ...
      .event-status-label {
        width: 64px;
        height: 65px;
        background: url("~@/assets/images/event_hand_status_label.png")
          no-repeat;
        position: absolute;
        right: 130px;
        top: 0px;
